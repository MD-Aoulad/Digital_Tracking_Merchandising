...
export const AttendanceProvider: React.FC<AttendanceProviderProps> = ({ children }) => {
  ...
  const setupWebSocketListeners = () => {
    onWebSocketEvent('attendance_update', (data) => {
      if (data.userId === 'current-user') { // Replace with actual user ID
        dispatch({ type: 'SET_CURRENT_STATUS', payload: data.status });
        toast.success(`Status updated: ${data.status}`);
      }
    });

    onWebSocketEvent('team_update', (data) => {
      dispatch({ type: 'SET_TEAM_STATUS', payload: data.teamStatus });
    });

    onWebSocketEvent('approval_request', (data) => {
      dispatch({ type: 'ADD_APPROVAL_REQUEST', payload: data });
  ...
  const cleanupWebSocketListeners = () => {
    offWebSocketEvent('attendance_update', () => {});
    offWebSocketEvent('team_update', () => {});
    offWebSocketEvent('approval_request', () => {});
  };
