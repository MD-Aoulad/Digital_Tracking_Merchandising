...
const MemberManagement: React.FC<MemberManagementProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!isEditing) {
      ...
      try {
        if (!formData.email || !formData.name || !password) {
        ...
        const role = formData.role === MemberRole.ADMIN ? 'admin' : 'employee';
    ...
    // Editing existing member (no password change)
          ...
          <div className="mb-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  Email Address *
                  <span className="ml-1 text-gray-400" title="Enter the employee's email address. This will be used for login.">?</span>
                </label>
                <input
                  type="email"
